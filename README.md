...
- INDEL はホモポリマーや繰り返し配列の近くに多く、リード端のソフトクリップが片方の向きで
  ...
  INDEL に流用したことが原因。
- SNP は変化なし。追加した SNP 用フィルタ（QUAL、MQRankSum、ReadPosRankSum）は
  信頼領域内の結果を1件も変えなかった。
...
**SNP の見逃しの下限**
- SNP の見逃しはどの条件でも約136〜139件でほぼ一定。
  フィルタや補正では回復しない見逃しが存在する（下記の事例2）。
...
### 見逃し（FN）の分析
Day 2 の SNP 見逃しを、次の3種類に分類した（`day3/fn_triage.txt`）。

| 分類 | 意味 |
|---|---|
| 未コール | その位置に何も呼ばれていない |
| フィルタ落ち | 呼ばれたが FILTER で除外された |
| PASS だが不一致 | 遺伝子型の誤り（ヘテロ / ホモ）や表記の違い |

IGV で確認した見逃し地点はいずれも領域先頭の 14 Mb 付近で、深度は 18 Mb 付近
（約38x）の半分以下だった。位置分布は `day3/fn_triage.txt` を参照。
...
- 短いリードの WGS には原理的に見えない場所があり、感度には上限がある。
- 仮説は必ず別のデータで検証する（今回は元の 300x データで確認した）。
